...
break on stime skip 1 on mod on sqlid_cn on sqlxid on sqlstart
select 	 to_char(sample_time, 'HH24:MI:SS')     stime
	,lpad(session_id,4,' ')||','||lpad(session_serial#,5,' ')||decode(substr(blocking_session_status,1,1),'V','<',' ')||lpad(blocking_session,4,' ')	sidser
	,module					mod
...
where	 lower(event) like lower('%&&event_pattern%')
and	 sample_time >= sysdate - &&last_x_min/1440
